...
(n_x, n_u, n_uncertainty, norm_type, N_horizon, eps_nom, modes, Hx, hx, Hu, hu, Q, R, x_ref, u_ref, x0_ini) = SYS_PARAMS.values()
...
uncertainty_domain = pt.box2poly(np.array([lb_ini, ub_ini]).transpose())
...
regions_ini = [{'A': uncertainty_domain.A, 'b': uncertainty_domain.b.reshape(-1, 1)}]
regions = regions_ini
...
for i in range(N_exp):
    ...
    for t in range(T_cl):
        ...
        if cost_at_t > stab_threshold or REDUCE_K is False:
            # Kmeans is done online (clustering forecast)
            K = K_to_split
            samples_for_partition = generator.get_samples(1000, N_horizon, t_ini=t)
            time_partitioning = time.time()
            regions = generator.my_kmeans_partition(samples_for_partition, K, uncertainty_domain, N_horizon * n_uncertainty)
            time_online_partitioning_matrix[i, t] = time.time() - time_partitioning
        else:
            K, regions = 1, regions_ini
        ...
        p_hat, nominal_scen, polytopes, regions_new, _, clusters = generator.compute_partition_elements(samples, regions, None,
                                                                                                     N_samples)
        ...
        pp_controller.set_tightening(nominal_scen, regions=regions_new)
...
if SAVE:
    ...
    file_name = 'closed_loop_kmns_K' + str(K_to_split) + '_' + str(idx_experiment) + '.pkl'
    ...
    variables_to_save = {'cost_matrix': cost_matrix, 'solver_time_matrix': time_solver_matrix,
                         'k_means_time_matrix': time_online_partitioning_matrix,
